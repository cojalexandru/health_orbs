...
public class FileUtils{
    ...
    public static final Path MOD_ROOT = ModList.get().getModFileById(PickableOrbs.MOD_ID).getFile().getFilePath();
    ...
    public static void streamFilesAndParse(Path directoryPath, BiConsumer<Reader, String> instructions, String errorMessage) {
        try (Stream<Path> zipStream = Files.walk(directoryPath);
            ...
            PickableOrbs.LOGGER.error(errorMessage, e);
        }
    }
    ...
    public static void setupDefaultFiles(String dataPath, Path targetPath) {
        if (Files.isRegularFile(MOD_ROOT)) {
            try (FileSystem fileSystem = FileSystems.newFileSystem(MOD_ROOT, ClassLoader.getSystemClassLoader())) {
                ...
                PickableOrbs.LOGGER.error("Could not load source {}!!", MOD_ROOT);
                e.printStackTrace();
            }
    ...
    try {
        ...
        PickableOrbs.LOGGER.warn("File not found: {}", path);
    }
    }
            ...
            zf.stream()
                    ...
                    .forEach(zipEntry -> {
                        try {
                            ...
                            PickableOrbs.LOGGER.error("Could not parse zip entry: {}", zipEntry.getName());
                        }
                    });
        } catch (IOException e) {
            PickableOrbs.LOGGER.warn("Could not read Zip File: {}", file.getFileName());
        }
    }
            ...
            sourceStream.filter(f -> f.getFileName().toString().endsWith(JSON))
                    .forEach(path -> {
                        try {
                            ...
                            PickableOrbs.LOGGER.error("Could not copy file: {}, Target: {}", path, targetPath);
                        }
                    });
        } catch (IOException e) {
            PickableOrbs.LOGGER.error("Could not stream source files: {}", source);
        }
    }
}
